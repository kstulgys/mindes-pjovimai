...
function SideNavBar() {
  ...
  return (
    ...
    "
        />
        <ManuItemModal icon={FiCoffee} title="Contacts" buttonsText="Send a message" text="How did you enjoy the app? Send us a message!" />
    ...
    <>
      <Button title={title} onClick={handleOnOpen} variant="unstyled" _hover={{ bg: "blue.200" }}>
        <Stack direction={"row"}>
          ...
          <Text justify="center">{IconTitle}</Text>
        </Stack>
      </Button>
      <Modal isOpen={isOpen} onClose={onClose} isCentered size="xl">
        ...
        <ModalContent>
          ...
          <ModalFooter>
            {buttonsText ? (
              <Button _hover={{}} bg="gray.900" color="white" onClick={title == "Contacts" ? sendAMessage : handleLogout}>
                {buttonsText}
              </Button>
            ) : (
              <></>
